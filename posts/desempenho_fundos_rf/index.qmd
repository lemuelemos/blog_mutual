...
con <- DBI::dbConnect(
  duckdb::duckdb(),
  dbdir = "dados_informes/fundos_db.duckdb",
  read_only = FALSE
)
...
meses_download <- format(
  seq(as.Date("2021-01-01"), as.Date("2025-01-01"), by = "month"),
  "%Y%m"
)
...
furrr::future_walk(
  ...
  function(ano) {
    ...
    destfile <- paste0(
      "./posts/desempenho_fundos_rf/dados_informes/",
      ano,
      ".zip"
    )
...
furrr::future_walk(zipfiles, function(path) {
  unzip(path, exdir = "./posts/desempenho_fundos_rf/dados_informes/")
})
...
#| code-fold: false
#|
fundos_175_anbima <- readxl::read_excel(
  "fundos-175-caracteristicas-publico-21-07-2025-00-06-47.xlsx"
)
...
DBI::dbWriteTable(con, "extrato_fi", extrato_fi)
```
...
fundos_rf %>%
  ...
  arrange(Competência) %>%
  mutate(Competência = as.Date(Competência), `Custódia` = `Custódia` / 1e9) %>%
  group_by(`Tipo ANBIMA`) %>%
  e_charts(Competência) %>%
