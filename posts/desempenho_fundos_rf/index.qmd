...
```{r grafico de captação,cache=TRUE}
fundos_rf %>%
  filter(!is.na(POLIT_INVEST)) %>%
  mutate(Competência = as.Date(as.yearqtr(DT_COMPTC))) %>%
  filter(Competência >= as.Date("2010-01-01")) %>%
  summarise(
  ...
  e_line(`Captação líquida acumulada`, name = "Captação líquida acumulada", color = "purple") %>%
  e_title(
